...
class PPAKPATCHER_API FPPakPatcherMemory
{
public:

	explicit FPPakPatcherMemory(const int64 PreAllocateBytes = 0);
	~FPPakPatcherMemory();
	...
	/** Grow memory but not change size.*/
	void Reserve(int64 Size);

private:

	/** Non-copyable */
	FPPakPatcherMemory(const FPPakPatcherMemory&) = delete;
	FPPakPatcherMemory& operator=(const FPPakPatcherMemory&) = delete;
